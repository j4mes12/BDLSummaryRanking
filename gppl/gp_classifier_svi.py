...
def _gradient_terms_for_subset(
    K_mm, invK_mm, kernel_derfactor, kernel_operator, common_term, ls_d, coords, s
):
    if kernel_operator == "*":
        dKdls = (
            K_mm * kernel_derfactor(coords, coords, ls_d, operator=kernel_operator) / s
        )
...
class GPClassifierSVI(GPClassifierVB):
    ...
    def __init__(
        ...
        super(GPClassifierSVI, self).__init__(
            ninput_features,
            z0,
            shape_s0,
            rate_s0,
    ...
    def _init_s(self):
        ...
        if not self.fixed_s:
            ...
            self.rate_s = self.rate_s0 + 0.5 * (
                ...
                + self.ninducing * self.rate_s0 / self.shape_s0
            )
    ...
    def _choose_inducing_points(self):
        ...
        )  # number of inducing points in each stochastic update
        ...
        if self.inducing_coords is None and (
            self.ninducing > self.n_locs or self.cov_type == "diagonal"
        ):
            ...
            if self.obs_coords.shape[0] > 20 * self.ninducing:
                coords = self.obs_coords[
                    np.random.choice(
                        self.obs_coords.shape[0], 20 * self.ninducing, replace=False
                    ),
                    :,
                ]
            else:
        ...
        if self.K_mm is None:
            self.K_mm = self.kernel_func(
                self.inducing_coords, self.ls, operator=self.kernel_combination
            )
        ...
        self.u_Lambda = np.zeros(
            (self.ninducing, self.ninducing), dtype=float
        )  # observation precision at inducing points
        self.uS = (
            self.K_mm * self.rate_s0 / self.shape_s0
        )  # initialise properly to prior
    ...
    def _update_jacobian(self, G_update_rate=1.0):
        ...
        if (
            G_update_rate == 1
            or not len(self.G)
            or self.G.shape != J.shape
            or self.changed_selection
        ):
    ...
    def _logpt(self):
        logrho, lognotrho, _ = self._post_sample(
            self.obs_f, self.obs_v, expectedlog=True
        )
    ...
    def lowerbound_gradient(self, dim):
        ...
        common_term = (self.um_minus_mu0.dot(self.um_minus_mu0.T) + self.uS).dot(
            self.s * self.invK_mm
        ...
        if len(self.ls) > 1:
            gradient = get_parallel()(n_jobs=num_jobs, backend="threading")(
                delayed(_gradient_terms_for_subset)(
                    self.K_mm,
                    self.invK_mm,
                    ...
                    common_term,
                    self.ls[dim],
                    self.inducing_coords[:, dim : dim + 1],
                    self.s,
                )
                for dim in dims
            )
    ...
    def _update_f(self):
        ...
        w_i = np.sum(self.obs_total_counts) / float(
            np.sum(self.obs_total_counts[self.data_obs_idx_i])
        )  # self.obs_f.shape[0] / float(self.obs_f_i.shape[0])
        ...
        self.u_invS = (1 - rho_i) * self.prev_u_invS + rho_i * (
            w_i * Lambda_i + self.invKs_mm
        )
        self.u_Lambda = (1 - rho_i) * self.prev_u_Lambda + rho_i * w_i * Lambda_i

        # use the estimate given by the Taylor series expansion
        z0 = self.forward_model(self.obs_f, subset_idxs=self.data_idx_i) + self.G.dot(
            self.mu0_i - self.obs_f_i
        )
        y = self.z_i - z0

        # Variational update to theta_1 is (1-rho)*S^-1m + rho*beta*K_mm^-1.K_mn.y
        self.u_invSm = (1 - rho_i) * self.prev_u_invSm + w_i * rho_i * (
            Lambda_factor1 / Q
        ).dot(y)
        ...
        self.obs_f, self.obs_v = self._f_given_u(
            self.covpair, self.mu0, 1.0 / self.s, full_cov=False
        )
    ...
    def init_inducing_points(
        self, inducing_coords, K_mm=None, invK_mm=None, K_nm=None, V_nn=None
    ):
    ...
    def _update_sample_idxs(self):
        if not self.fixed_sample_idxs:
            self.data_idx_i = np.sort(
                np.random.choice(self.n_locs, self.update_size, replace=False)
            )
        self.data_obs_idx_i = self.data_idx_i
    ...
    def _expec_f_output(
        ...
        """
        if not self.use_svi:
            return super(GPClassifierSVI, self)._expec_f_output(
                Ks_starstar, Ks_star, mu0, full_cov, reuse_output_kernel
            )
        ...
        if self.covpair_out is None or not reuse_output_kernel:
            ...
            if (
                reuse_output_kernel
            ):  # we only overwrite this if the reuse flag indicates we should
