...
def get_idf_weights(ref_vecs):
    ...
    dfs = [np.sum(sim_matrix[i]) - 1.0 for i in range(len(ref_vecs))]
    dfs = [1.0 * d / (len(ref_vecs) - 1) for d in dfs]
    dfs = [(d + 1.0) / 2.0 for d in dfs]
    idf = [-1.0 * np.log(df) for df in dfs]
    return idf
...
def get_ref_vecs(doc_sent_vecs, doc_sent_weights, info_dic):
    ...
    for dd in docs:
        ref_dic[dd] = [
            i
            for i in range(len(doc_sent_weights))
            if doc_sent_weights[i] >= 0.1 and info_dic[i]["doc"] == dd
        ]
    vecs = []
    for dd in ref_dic:
...
def get_sim_metric(
    summ_vec_list, doc_sent_vecs, doc_sent_weights, info_dic, method="cos"
):
    ...
    if method == "cos":
        ...
        dvec = np.matmul(
            np.array(doc_sent_weights).reshape(1, -1), np.array(doc_sent_vecs)
        )
    ...
    # bert-score, quicker to run and gives similar performance to mover-bert-score
        ...
        idf_recall = np.dot(np.max(sim_matrix, axis=1), idf_weights) / np.sum(
            idf_weights
        )
        ...
        if recall + precision == 0:
            ...
            idf_f1 = 2.0 * idf_recall * precision / (idf_recall + precision)
        if method.lower().startswith("f"):
            ...
            return np.dot(
                np.array(np.max(sim_matrix, axis=1)), np.array(weights)
            ) / np.sum(weights)
        else:
            return None
    ...
    for dd in docs:
        ...
        for i, sent in enumerate(dd[1]):
            sent_index[cnt] = {
                ...
                "inside_doc_idx": i,
                "doc_len": doc_len,
                "inside_doc_position_ration": i * 1.0 / doc_len,
            }
            cnt += 1
    ...
    for i, rr in enumerate(refs):
        ...
        for j, sent in enumerate(ref_sents):
            sent_index[cnt] = {
                ...
                "text": sent,
                "inside_doc_idx": j,
                "doc_len": len(ref_sents),
                "inside_doc_position_ration": j * 1.0 / len(ref_sents),
            }
            cnt += 1
            all_sents.append(sent)
    all_vecs = None
