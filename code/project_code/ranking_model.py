...
https://github.com/UKPLab/tacl2020-interactive-ranking/blob/master/BERT_cQA.py
and adjusted to rank text summaries instead of cQA
"""
...
def construct_single_item_dataset(dataframe):
    ...
    for idx, qid in enumerate(dataframe.index):
        ...
        if qid not in goldids:
            print(
                "Didn't find the goldid "
                + f"in the list of candidates for q {qid}. "
...
if __name__ == "__main__":
    ...
    for q in tr_qa_pairs:
        qa_length = len(q[0].split(" "))
        qmax = qa_length if qa_length > qmax else qmax
        if qa_length > 512:
            noverlength += 1
