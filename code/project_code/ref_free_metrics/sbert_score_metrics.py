...
def get_sbert_score(
    ref_token_vecs, ref_tokens, summ_token_vecs, summ_tokens, sim_metric
):
    recall_list = []
    precision_list = []
    f1_list = []
    ...
    if "recall" in sim_metric:
        ...
        for i in range(len(summ_token_vecs)):
            if i in empty_summs_ids:
                ...
                scores.append(np.mean(recall_list[:, i]))
        return scores
        ...
        for i in range(len(summ_token_vecs)):
            if i in empty_summs_ids:
                ...
                scores.append(np.mean(precision_list[:, i]))
        return scores
