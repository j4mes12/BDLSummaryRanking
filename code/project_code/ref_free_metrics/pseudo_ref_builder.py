...
def get_top_sim_weights(sent_info, full_vec_list, max_sim_value):
    ...
    weights = [0.0] * len(sent_info)
    for dn in doc_names:
        doc_idx = [k for k in sent_info if sent_info[k]["doc"] == dn]
        sim_matrix = cosine_similarity(
            np.array(full_vec_list)[doc_idx], np.array(full_vec_list)[doc_idx]
        )
        nn = get_doc_simtop(sim_matrix, max_sim_value)
        for i in range(np.min(doc_idx), np.min(doc_idx) + nn):
            weights[i] = 1.0
    return weights
...
def get_other_weights(full_vec_list, sent_index, weights, thres):
    ...
    for sg in subgraphs:
        if any(weights[n] >= 0.9 for n in sg):
            continue  # ignore the subgraph similar to a top sentence
        if len(set([sent_index[n]["doc"] for n in sg])) < 2:
            continue  # must appear in multiple documents
        for n in sg:
            weights[n] = 1.0 / len(sg)
        # print(sg,'added to weights')


def graph_centrality_weight(similarity_matrix):
    weights_list = [
        np.sum(similarity_matrix[i]) - 1.0 for i in range(similarity_matrix.shape[0])
    ]
    return weights_list
...
def get_global_graph_weights(sent_info_dic, sent_vecs, num, top_n, extra_ratio):
    ...
    if top_n is not None:
        top_ids = [
            i for i in sent_info_dic if sent_info_dic[i]["inside_doc_idx"] < top_n
        ]
        adjusted_weights = [
            w * extra_ratio if j in top_ids else w for j, w in enumerate(raw_weights)
        ]
    else:
...
def get_indep_cluster_weights(sent_info_dic, sent_vecs):
    ...
    for dname in doc_names:
        ...
        for cc in centers:
            wanted_ids.append(sums.index(np.sum(cc)))
    print("indep cluster, pseudo-ref sent num", len(wanted_ids))
    weights = [1.0 if i in wanted_ids else 0.0 for i in range(len(sent_vecs))]
    return weights
...
def get_global_cluster_weights(sent_vecs):
    ...
    ids = []
    for cc in centers:
        ids.append(sums.index(np.sum(cc)))
    assert len(ids) == len(centers)
    weights = [1.0 if i in ids else 0.0 for i in range(len(sent_vecs))]
    return weights
