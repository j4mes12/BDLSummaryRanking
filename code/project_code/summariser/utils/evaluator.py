...
def evaluateReward(learnt_values, ref_values, short=False, top_answer=None):
    ...
    metrics_dic["rank_of_best"] = float(
        len(learnt_values)
        - np.argwhere(ranked_items == np.argmax(ref_values)).flatten()[0]
    )
...
def evaluateSummary(cand, model, len=100):
    ...
    dic["ROUGE-L"] = rl
    dic["ROUGE-SU4"] = rsu4
    dic["WeightedSum"] = 3.33 * (r1 / 0.47 + r2 / 0.212 + rsu4 / 0.185)
    return dic
...
def plotAgreement(truer, learntr, bin_num=20, plot=True, sample_num=5000):
    ...
    for i in range(0, len(true_rewards) - 1):
        for j in range(i + 1, len(true_rewards)):
            ...
            cross_entropy[idx] -= true_label * np.log(prob) + (1 - true_label) * np.log(
                1 - prob
            )
    ...
    norm_cross_entropy = (
        np.array(cross_entropy[total_case > 0]) / total_case[total_case > 0]
    )
    ...
    for i in range(len(agreement_ratio)):
        if (
            probs is not None
            and not np.isnan(agreement_ratio[i])
            and not np.isnan(probs[i])
        ):
