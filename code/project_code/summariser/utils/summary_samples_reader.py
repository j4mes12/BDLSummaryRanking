...
def getDBSRewardsFromWeights(summary_list, weights_dir, vector_path):
    # get vectors
    _, vector_list, _ = readSyntheticSampleVectors(
        summary_list, vector_path, False, True, False
    )
...
def getRewardsFromWeights(
    summary_list, weights_dir, vector_path, sample_num, model_num
):
    # get vectors
    _, vector_list, _ = readSyntheticSampleVectors(
        summary_list, vector_path, False, True, False
...
def readAccPretrainedRewards(acts_list, topic, data_set):
    print(
        "\n-----Start reading pre-trained acc-rewards. Target num: {}-----".format(
            len(acts_list)
        )
    )
    # read in accumulated rewards
    reward_path = os.path.join(
        "/home/gao/PycharmProjects/sampling_summaries/pretrained_rewards",
        data_set,
        topic,
    )
    cnt = 0
    ...
    for line in ff.readlines():
        if "actions:" in line:
            ...
            print(
                "!!!!!ERROR in reading pretrained rewards! Count:{}\n Path: {}".format(
                    cnt, reward_path
                )
            )
    ff.close()
    ...
    print(
        "{} pretrained acc-rewards have been read and normalised".format(len(norm_list))
    )
    return norm_list
...
def readSumVecPretrainedRewards(summary_list, topic, data_set):
    ...
    print(
        "\n-----Start reading pre-trained rewards. Target num: {}-----".format(
            sample_num
        )
    )
    vector_strs_dir = os.path.join(
        "/home/gao/workspace/v43-12Sep/summarizer/sampled-summaries-vectors-strs",
        data_set,
        topic,
    )
    vector_file_path = os.path.join(
        vector_strs_dir, "optimal", "state{}-base{}-block{}".format("NGRAM", 100, 6)
    )
    _, vector_list, _ = readSyntheticSampleVectors(
        summary_list, vector_file_path, False, True, False
    )

    # read weights
    weights = []
    weights_path = os.path.join(
        "/home/gao/workspace/v43-12Sep/summarizer/pretrained_rewards/readin_weights",
        topic,
    )
    ...
    print(
        "{} pre-trained rewards are read and normalised.".format(len(norm_reward_list))
    )
...
def readIntermediateSampleVectors(
    summary_act_list,
    path,
    vec_length,
    inter_vectors_wanted=True,
    final_vector_wanted=True,
):
...
def readShortAccSampleVectors(
    summary_act_list,
    path,
    vec_length,
    str_wanted=True,
    acc_vector_wanted=True,
    final_vector_wanted=True,
    reward_wanted=True,
):
    print("\n-----Start reading vectors from: {}-----".format(path))
    ...
    for line in file.readlines():
        ...
        if "AYGactionsAYG:" in line:
            ...
            if acc_vector_wanted:
                vec = reconstructFullVectorFromShort(
                    vec_length, line[len("AYGacc_vectorAYG:") :]
                )
                acc_vector_list.append(vec)
        elif "AYGfinal_vectorAYG:" in line:
            if final_vector_wanted:
                vec = reconstructFullVectorFromShort(
                    vec_length, line[len("AYGfinal_vectorAYG:") :]
                )
                final_vector_list.append(vec)
...
def readSyntheticSampleVectors(
    summary_act_list, path, str_wanted=True, vector_wanted=True, reward_wanted=True
):
    print("\n-----Start reading vectors from: {}-----".format(path))
    summary_list = []
    vector_list = []
...
def readDbsOptimalSummariesAndValues(sample_num, dir, topic, only_act=False):
    print(
        "\n-----Start reading samples in topic {}. Target sample num: {}-----".format(
            topic, sample_num
        )
    )
...
def readSummariesAndValues(sample_num, dir, topic, reward_type, only_act=False):
                ...
                if "model" not in line and not flag:
                    ...
                    if "action" in line and idx == 0:
                        ...
                        if "DUC" in dir:
                            ...
                            print(
                                "Error in readSummariesAndValues! DUC or DBS are not in the directory {}".format(
                                    dir
                                )
                            )
                            exit(1)
    ...
    if reward_type == "synthetic":
        ...
        for cnt in range(len(summary_value_list)):
            ...
            for idx in range(model_num):
                temp_row.append(
                    10.0 * (summary_value_list[cnt][idx] - minvs[idx]) / ranges[idx]
                )
            norm_value_list.append(temp_row)
...
def getValueListFromGpplModel(
    summary_list, vector_path, model_dir, setting, model_name
):
    tt = setting.split("_")
    ...
    if file_name not in os.listdir(model_dir):
        print(
            "ERROR! No file in dir {} corresponds to model {}".format(
...
def getGpplLearntRewards(
    summary_list, data_set, topic, learnt_rewards_setting_name, noisy, model_names
):
    if "DUC" in data_set:
        base_length = 200
        block_num = 1
    ...
    if noisy and "DUC" in data_set:
        gppl_model_dir = os.path.join(
            "/home/gao//PycharmProjects/gppl_rankers/v1_12Feb18_alQuery/learnt_model",
            "10PercentNoisyFeedback",
            data_set,
            topic,
    ...
    if "DUC" in data_set:
        vector_path = os.path.join(
            "/home/gao/PycharmProjects/sampling_summaries/2_vectors",
            data_set,
            topic,
            "synthetic",
            "stateNGRAM-base{}-block{}".format(base_length, block_num),
        ...
        for model in model_names:
            value_list = getValueListFromGpplModel(
                summary_list,
                vector_path,
                gppl_model_dir,
...
def getLearntRewards(
    summary_list, data_set, topic, learnt_rewards_setting_name, noisy, model_idx=0
):
    if "DUC" in data_set:
        base_length = 200
        block_num = 1
    ...
    if noisy and "DUC" in data_set:
        learnt_weights_dir = os.path.join(
            "/home/gao/PycharmProjects/sampling_summaries/al_learnt_rewards_weights",
            "10PercentNoisyFeedback",
            data_set,
            topic,
            "stateNGRAM-base{}-block{}".format(base_length, block_num),
        ...
        learnt_weights_path = os.path.join(
            "/home/gao/PycharmProjects/sampling_summaries/al_learnt_rewards_weights",
            "10PercentNoisyFeedback",
            data_set,
            topic,
    ...
    if "DUC" in data_set:
        ...
        models = sorted(os.listdir(learnt_weights_dir))
        model_num = 0
        for model in models:
            if model[0] != ".":
                model_num += 1
        new_learnt_value_list = getRewardsFromWeights(
            summary_list, learnt_weights_dir, vector_path, len(summary_list), model_num
        )
    else:
        vector_path = os.path.join(
            "/home/gao/PycharmProjects/sampling_summaries/2_vectors",
        ...
        new_learnt_value_list = getDBSRewardsFromWeights(
            summary_list, learnt_weights_path, vector_path
        )

    return new_learnt_value_list


def getGpplMixedLearntRewards(
    sample_num, samples_dir, topic, data_set, setting_name, model_names, noisy=False
):
    # get learnt rewards
    if "DUC" in data_set:
        act_list = readSummariesAndValues(
            sample_num, samples_dir, topic, "optimal", True
        )
    else:
        exit(1)  ###TODO: DBS

    learnt_rewards_list = getGpplLearntRewards(
        act_list, data_set, topic, setting_name, noisy, model_names
    )
...
def getMixedLearntRewards(
    ...
    if "DUC" in data_set:
        act_list = readSummariesAndValues(
            sample_num, samples_dir, topic, reward_type, True
        )
        topic_model = topic
    else:
        topic_model = os.path.join(topic, "model_{}".format(model_idx))
        if reward_type == "optimal":
            act_list = readDbsOptimalSummariesAndValues(
                sample_num, samples_dir, topic_model, True
            )
        else:
            act_list = readSummariesAndValues(
                sample_num, samples_dir, topic_model, reward_type, True
            )

    learnt_rewards_list = getLearntRewards(
        act_list, data_set, topic, setting_name, noisy, model_idx
    )
    pre_rewards_list = readAccPretrainedRewards(act_list, topic_model, data_set)

    print(
        "length of pre_rewards: {}; length of learnt rewards: {}".format(
            len(pre_rewards_list), len(learnt_rewards_list)
        )
    )
    mixed_rewards_list = mixingPretrainedLearntRewards(
        pre_rewards_list, learnt_rewards_list, tdf
    )
