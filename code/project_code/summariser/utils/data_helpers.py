...
def remove_stopwords(words, stoplist):
    ...
    return [
        token for token in words if not (token.startswith(PUNCT) or token in stoplist)
    ]
...
def extract_ngrams2(sentences, stemmer, language, N=2):
    ...
    for sent in sentences:
        sent = re.sub(
            "[-](,?\s)", "\\1", sent
        )  # case where magister- has to be handled
...
def untokenize(tokens):
    ...
    text = re.sub(" ([\.;,-?!])$", "\g<1>", text)
    text = re.sub(" _ (.+) _ ", " _\g<1>_ ", text)
    text = re.sub(" \$ ([\d\.]+) ", " $\g<1> ", text)
    text = text.replace(" ' ", "' ")
    text = re.sub("([\W\s])\( ", "\g<1>(", text)
    text = re.sub(" \)([\W\s])", ")\g<1>", text)
    text = text.replace("`` ", "``")
    text = text.replace(" ''", "''")
    text = text.replace(" n't", "n't")
    text = re.sub('(^| )" ([^"]+) "( |$)', '\g<1>"\g<2>"\g<3>', text)
...
def load_w2v_embeddings(embeddings_path, language, oracle_type):
    ...
    if oracle_type.startswith("active_learning"):
        if language == "english":
            ...
            embeddPath = path.normpath(
                path.join(embeddings_path, "english/glove.6B.300d.txt")
            )
        ...
        if language == "german":
            embeddPath = path.normpath(
                path.join(
                    embeddings_path, "german/2014_tudarmstadt_german_50mincount.vec"
                )
            )
