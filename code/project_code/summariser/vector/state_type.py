...
class State:
    ...
    def getSelfText(self, top_ngrams, sentences):
        pass
    ...
    def getStateVector(self, draft_list, draft_index_list, top_ngrams, sentences):
        ...
        :param draft_list: a list of sentences, included in the current draft
         summary
        :param draft_index_list: the indices of the included sentences
        :param top_ngrams: top n-grams for all the original documents
        :param sentences: all sentences information, used to find positions
         information
        :param tfidf: decides to use the Japan version (state_type==True) or
         the REAPER version
        ...
        if (
            len(draft_list) == 0 or draft_list is None
        ):  # or draft_length>self.sum_token_length:
        ...
        draft_ngrams = util.extract_ngrams_count(
            draft_list, self.stemmer, self.language, self.stoplist
        )
        ...
        if sent_num >= self.state_length_computer.block_num:
            ...
            assert index == -1 + self.state_length_computer.getIndexUntilSentNum(
                sent_num + 1
            )
    ...
    def getSimilarity(self, tokens1, sentences2, fullDoc=False):
        ...
        # sentences2 is a list of sentences,
    ...
    def getNewStateVec(self, new_sent_id, top_ngrams, sentences):
        temp_draft_summary_list = self.draft_summary_list + [
            sentences[new_sent_id].untokenized_form
        ]
        draft_index_list = self.historical_actions + [new_sent_id]
        return self.getStateVector(
            temp_draft_summary_list, draft_index_list, top_ngrams, sentences
        )
    ...
    def removeOverlengthSents(self, sents):
        ...
        for sent_id in self.available_sents:
            if sent_id == 0:
                continue
            if (
                len(sents[sent_id - 1].untokenized_form.split(" "))
                > self.sum_token_length - self.draft_summary_length
            ):
    ...
    def getOptimalTerminalRougeScores(self, model):
        ...
        rouge = Rouge(res.ROUGE_DIR, res.BASE_DIR, True, True)
        R1, R2, R3, R4, RL, RSU = rouge(
            " ".join(self.draft_summary_list), [model], self.sum_token_length
        )
        rouge.clean()
        return [R1, R2, R3, R4, RL, RSU]

    def getTerminalReward(
        self, sentences, sentences_stemmed_aggreate, sent2tokens, sim_scores
    ):
        ...
        for i in range(len(self.historical_actions)):
            ...
            if (sent_idx, -1) in sim_scores:
                ...
                sim_score = (
                    self.getSimilarity(
                        ...
                        sentences_stemmed_aggreate,
                        True,
                    )
                    + 1.0 / sentences[sent_idx].position
                )
            ...
            for j in range(i):
                ...
                if (sent_idx, idx2) in sim_scores:
                    ...
                    red_score = self.getSimilarity(
                        " ".join(sent2tokens(self.draft_summary_list[j])),
                        " ".join(sent2tokens(self.draft_summary_list[i])),
                    )
        ...
        return (
            relatedness_score * self.reward_lambda
            - (1 - self.reward_lambda) * redundant_score
        )
...
class StateLengthComputer:
    def __init__(self, block_num, base_length, sent_num):
        ...
        for i in range(block_num):
            self.lengths.append(
                int(base_length * np.log10(self.ncr(sent_num, i + 1)) * 1.0 / base_num)
                + 5
            )
...
if __name__ == "__main__":
    ...
    sent_num = 400
    print(
        "block num: {}; sentence num: {}; "
        "the summary of length 1 will have {}-dimension states.".format(
            block_num, sent_num, base_num
        )
    )
