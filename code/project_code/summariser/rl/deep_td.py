...
class DeepTDAgent:
    ...
    def produceSummary(self):
        state = State(
            self.vectoriser.sum_token_length,
            self.vectoriser.base_length,
            len(self.vectoriser.sentences),
            self.vectoriser.block_num,
            self.vectoriser.language,
        )
    ...
    def getGreedySent(self, state):
        ...
        for act_id in state.available_sents:
            ...
            if act_id == 0:
                current_state_vec = state.getSelfVector(
                    self.vectoriser.top_ngrams_list, self.vectoriser.sentences
                )
                vec_variable = Variable(
                    torch.from_numpy(np.array(current_state_vec)).float()
                )
                terminate_reward = self.deep_model(
                    vec_variable.unsqueeze(0)
                ).data.numpy()[0][0]

            # otherwise, the reward is 0, and value-function can be computed using the weight
            else:
                temp_state_vec = state.getNewStateVec(
                    act_id - 1,
                    self.vectoriser.top_ngrams_list,
    ...
    def train(self, summary_actions, summary_value):
        ...
        current_vec = state.getStateVector(
            state.draft_summary_list,
    ...
    def deepTrain(self, vec_list, last_reward):
        value_variables = self.deep_model(
            Variable(torch.from_numpy(np.array(vec_list)).float())
        )
...
if __name__ == "__main__":
    ...
    for topic, docs, models in data:
        ...
        print(
            "\n=====UNTIL TOPIC {}, REWARD TYPE {}, EPISODE {}, STRICT {}=====".format(
                topic_cnt, reward_type, episode, rl_strict
            )
        )
