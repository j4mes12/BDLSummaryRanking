...
def test_pairwise_distances():
    ...
    for squared in [True, False]:
        ...
        res_pt = BatchHardTripletLoss._pairwise_distances(
            torch.from_numpy(embeddings), squared=squared
        )
...
def test_pairwise_distances_are_positive():
    ...
    for squared in [True, False]:
        res_tf = BatchHardTripletLoss._pairwise_distances(
            torch.from_numpy(embeddings), squared=squared
        )
...
def test_anchor_positive_triplet_mask():
    ...
    mask_tf_val = BatchHardTripletLoss._get_anchor_positive_triplet_mask(
...
def test_anchor_negative_triplet_mask():
    ...
    mask_tf_val = BatchHardTripletLoss._get_anchor_negative_triplet_mask(
        torch.from_numpy(labels)
    )
...
def test_simple_batch_all_triplet_loss():
    ...
    for squared in [True, False]:
        ...
        loss_tf_val, fraction_val = BatchHardTripletLoss.batch_all_triplet_loss(
            labels, embeddings, margin, squared=squared
        )
...
def test_batch_hard_triplet_loss():
    ...
    labels = np.random.randint(
        min_class, min_class + num_classes, size=(num_data)
    ).astype(np.float32)
    ...
    for squared in [True, False]:
        ...
        # Compute the loss in TF.
        loss_tf_val = BatchHardTripletLoss.batch_hard_triplet_loss(
            torch.from_numpy(labels),
            torch.from_numpy(embeddings),
            margin,
            squared=squared,
