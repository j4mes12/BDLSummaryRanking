...
import logging
from gppl.gp_classifier_vb import (
    coord_arr_to_1d,
    coord_arr_from_1d,
    temper_extreme_probs,
)
from gppl.gp_classifier_svi import GPClassifierSVI
...
def get_unique_locations(obs_coords_0, obs_coords_1, mu_0=None, mu_1=None):
    if issparse(obs_coords_0) or issparse(obs_coords_1):
        ...
        obs_coords = hstack(
            (obs_coords_0[uidxs_0], obs_coords_1[uidxs_1]), format="csc"
        )
        ...
        _, uidxs, pref_vu = np.unique(
            all_coord_rows, return_index=True, return_inverse=True
...
def pref_likelihood(fmean, fvar=None, subset_idxs=[], v=[], u=[], return_g_f=False):
    ...
    if np.any(fvar < 0):
        logging.warning(
            "There was a negative variance in the pref likelihood! %s" % fvar
        )
    ...
    phi = norm.cdf(
        g_f
    )  # the probability of the actual observation, which takes g_f as a parameter. In the
...
class GPPrefLearning(GPClassifierSVI):
    ...
    pref_v = (
        []
    )  # the first items in each pair -- index to the observation coordinates in self.obsx and self.obsy
    pref_u = (
        []
    )  # the second items in each pair -- indices to the observations in self.obsx and self.obsy
    ...
    def __init__(
        ...
        super(GPPrefLearning, self).__init__(
            ninput_features,
            mu0,
            shape_s0,
            rate_s0,
    ...
    def _init_prior_mean_f(self, z0):
        self.mu0_default = (
            z0  # for preference learning, we pass in the latent mean directly
        )
    ...
    def _init_obs_prior(self):
        ...
        m_prior, not_m_prior, v_prior = self._post_sample(
            mu0, f_prior_var, False, None, self.pref_v, self.pref_u
        )
    ...
    def _count_observations(self, obs_coords, _, poscounts, totals):
        ...
        if self.features is not None:
            ...
            # get unique keys
            all_ravelled_coords = np.concatenate(
                (ravelled_coords_0, ravelled_coords_1), axis=0
            )
            uravelled_coords, origidxs, keys = np.unique(
                all_ravelled_coords, return_index=True, return_inverse=True
            )
            ...
            (
                ...
                grid_obs_counts.nonzero()
            )  # coordinate key pairs with duplicate pairs removed

            nonzero_all = np.concatenate((nonzero_v, nonzero_u), axis=0)
            ukeys, pref_vu = np.unique(
                nonzero_all, return_inverse=True
            )  # get unique locations

            self.obs_uidxs = origidxs[
                ukeys
            ]  # indexes of unique observation locations into the original input data

            # Record the coordinates of all points that were compared
            self.obs_coords = coord_arr_from_1d(
                uravelled_coords[ukeys],
                obs_coords_0.dtype,
    ...
    def forward_model(
        self, fmean=None, fvar=None, subset_idxs=[], v=[], u=[], return_g_f=False
    ):
        """
        f - should be of shape nobs x 1
    ...
    def _compute_jacobian(self, f=None, data_idx_i=None):
        ...
        phi, g_mean_f = self.forward_model(
            f, return_g_f=True
        ...
        if (
            data_idx_i is not None
            and hasattr(self, "data_obs_idx_i")
            and len(self.data_obs_idx_i)
        ):
            obs_idxs = obs_idxs[:, data_idx_i]
            J = J[self.data_obs_idx_i, :]
            s = (self.pref_v[self.data_obs_idx_i, np.newaxis] == obs_idxs).astype(
                int
            ) - (self.pref_u[self.data_obs_idx_i, np.newaxis] == obs_idxs).astype(int)
        else:
            s = (self.pref_v[:, np.newaxis] == obs_idxs).astype(int) - (
                self.pref_u[:, np.newaxis] == obs_idxs
            ).astype(int)
    ...
    def fit(
        ...
        if (
            process_obs
            and input_type == "binary"
            and np.sum((pref_values_in_input < 0) | (pref_values_in_input > 1))
        ):
            raise ValueError(
                "Binary input preferences specified but the data contained the values %s"
                % pref_values_in_input
            )
        elif (
            process_obs
            and input_type == "zero-centered"
            and np.sum((pref_values_in_input < -1) | (pref_values_in_input > 1))
        ):
            raise ValueError(
                "Zero-centered input preferences specified but the data contained the values %s"
                % pref_values_in_input
            )
            ...
            raise ValueError(
                'input_type for preference labels must be either "binary" or "zero-centered"'
            )

        # TODO: bug fix: if the same object is reused with different set of items, there is a crash because K_nm is not renewed.

        super(GPPrefLearning, self).fit(
            (items1_coords, items2_coords),
            preferences,
            totals,
            process_obs,
            mu0=mu0,
    ...
    def set_training_data(
        ...
        """
        pref_values_in_input = np.unique(preferences)
        if input_type == "binary" and np.sum(
            (pref_values_in_input < 0) | (pref_values_in_input > 1)
        ):
            raise ValueError(
                "Binary input preferences specified but the data contained the values %s"
                % pref_values_in_input
        ...
        super(GPPrefLearning, self).set_training_data(
            (items1_coords, items2_coords),
            preferences,
            totals,
            mu0=mu0,
            K=K,
            init_Q_only=init_Q_only,
            features=item_features,
        )
    ...
    def _update_sample_idxs(self):
        ...
        )  # only consider the points that are really observed
        ...
        if not self.fixed_sample_idxs:
            ...
            while not np.sum(
                self.data_obs_idx_i
            ):  # make sure we don't choose indices that have not been compared
                if nobs > self.update_size:
                    self.data_idx_i = np.sort(
                        np.random.choice(nobs, self.update_size, replace=False)
                    )
                else:
                    self.data_idx_i = np.arange(nobs)

                self.data_idx_i = obs_idxs[self.data_idx_i]
                self.data_obs_idx_i = np.in1d(self.pref_v, self.data_idx_i) & np.in1d(
                    self.pref_u, self.data_idx_i
                )
        else:
            self.data_obs_idx_i = np.in1d(self.pref_v, self.data_idx_i) & np.in1d(
                self.pref_u, self.data_idx_i
    ...
    def predict(
        ...
        if return_var:
            if self.use_svi:
                _, _, v_post = self._post_sample(
                    self.mu0_output, K_star=self.K_star, v=item_0_idxs, u=item_1_idxs
                )
            else:
                _, _, v_post = self._post_sample(
                    f, K_star=C, v=item_0_idxs, u=item_1_idxs
                )
    ...
    def predict_pairs_from_features(
        self,
        out_feats=None,
        out_1_feats=None,
        mu0_out=None,
        ...
        :return p [, var]: probability that first item in pair is preferred to second item [, variance in the probability]
        """
        out_feats, item_0_idxs, item_1_idxs, mu0_out = get_unique_locations(
            out_feats, out_1_feats, mu0_out, mu0_out_1
        )
    ...
    def _post_sample(
        self, f_mean, f_var=None, expectedlog=False, K_star=None, v=None, u=None
    ):
        if v is None:
            v = self.pref_v
        if u is None:
        ...
        if K_star is not None and self.use_svi:
            ...
            f_samples = mvn.rvs(
                mean=self.um_minus_mu0.flatten(), cov=self.uS, size=1000
            ).T
            ...
            f_samples = np.random.normal(
                loc=f_mean, scale=np.sqrt(f_var), size=(N, 1000)
            )
        ...
        v_post[
            m_post * (1 - not_m_post) <= 1e-7
        ] = 1e-8  # fixes extreme values to sensible values. Don't think this is needed and can lower variance?
    ...
    def _post_rough(self, f_mean, f_cov=None, pref_v=None, pref_u=None):
        ...
        if f_cov is None:
            m_post = self.forward_model(
                f_mean, None, v=pref_v, u=pref_u, return_g_f=False
            )
        else:
