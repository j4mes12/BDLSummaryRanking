...
def get_parallel():
    ...
    os.environ["OMP_NUM_THREADS"] = str(
        num_cpus / max_no_jobs
    )  # the number of CPUs that would be available per thread
...
def stop_using_parallel():
    ...
    os.environ["OMP_NUM_THREADS"] = str(
        num_cpus
    )  # use all available CPUs for the OMP threading now that we have
...
def compute_distance(col, row):
    ...
    if issparse(col) or issparse(row):
        ...
        col = (
            col.A
        )  # we assume the column and row are taken from a larger sparse matrix so converting to nparray is ok
        row = row.A
...
def diagonal_from_raw_vals(vals, ls, vals2=None, operator="*"):
    ...
    for i in range(vals.shape[0]):
        ...
        K[i, :] = (np.sum(np.abs(vals[i : i + 1, :] - vals2), axis=1) == 0).astype(
            float
        )
    return K
...
def compute_median_lengthscales(items_feat, multiply_heuristic_power=1.0, N_max=3000):
    if items_feat.shape[0] > N_max:
        items_feat = items_feat[
            np.random.choice(items_feat.shape[0], N_max, replace=False)
        ]

    ndims = items_feat.shape[1]

    default_ls_value = get_parallel()(
        delayed(_dists_f)(items_feat[:, f], f) for f in range(ndims)
    )
    stop_using_parallel()  # increase the number of threads available per thread now we have only one thread
...
def check_convergence(
    ...
    if verbose:
        if np.isscalar(newval):
            logging.debug(
                "%s: %.5f, diff = %f at iteration %i" % (label, newval, diff, iter)
            )
        elif newval.size == 1:
            logging.debug(
                "%s: %.5f, diff = %f at iteration %i"
                % (label, newval.flatten()[0], diff, iter)
            )
        else:
    ...
    if (
        positive_only and diff < -10000 * conv_threshold
    ):  # ignore any small errors as we are using approximations
        logging.warning(
            "%s = %.5f, changed by %.5f (converged = %i) in iteration %i"
...
class GPClassifierVB(object):
    ...
    def __init__(
        ...
        if ls_initial is not None:
            self.n_lengthscales = len(
                ls_initial
            )  # can pass in a single length scale to be used for all dimensions
        else:
        ...
        self.rate_ls = (
            rate_ls  # analogous to a sum of changes between points at a distance of 1
        )
        ...
        self.kernel_combination = (
            kernel_combination  #  operator for combining kernels for each feature.
        )
    ...
    def _init_covariance(self):
        ...
        if self.kernel_func is not None and self.cov_type != "pre":
            ...
            self.K = self.kernel_func(
                self.obs_coords, self.ls, operator=self.kernel_combination
            )
            self.K += 1e-6 * np.eye(len(self.K))  # jitter
        elif self.K is None:
            logging.error(
                'With covariance type "pre", the kernel must be passed in when calling fit()'
            )
    ...
    def estimate_obs_noise(self):
        ...
        self.obs_mean = (self.obs_values + self.nu0[1]) / (
            self.obs_total_counts + nu0_total
        )
    ...
    def _init_obs_prior(self):
        ...
        if np.isscalar(mu0):
            n_locs = (
                1  # sample only once, and use the estimated values across all points
            )
        else:
            n_locs = len(mu0)

        f_samples = np.random.normal(
            loc=mu0, scale=np.sqrt(self.rate_s0 / self.shape_s0), size=(n_locs, 1000)
        )
    ...
    def _init_s(self):
        if not self.fixed_s:
            ...
            self.rate_s = self.rate_s0 + 0.5 * (
                np.sum((self.obs_f - self.mu0) ** 2)
                + self.n_locs * self.rate_s0 / self.shape_s0
            )
    ...
    def _count_observations(self, obs_coords, n_obs, poscounts, totals):
        ...
        if (
            obs_coords.shape[0] == self.ninput_features
            and obs_coords.shape[1] != self.ninput_features
        ):
            if obs_coords.ndim == 3 and obs_coords.shape[2] == 1:
                obs_coords = obs_coords.reshape(
                    (obs_coords.shape[0], obs_coords.shape[1])
                )
        ...
        uravelled_coords, origidxs, idxs = np.unique(
            ravelled_coords, return_index=True, return_inverse=True
        )
        ...
        nonzero_idxs = grid_obs_counts.nonzero()[
            0
        ]  # ravelled coordinates with duplicates removed
        ...
        self.obs_coords = coord_arr_from_1d(
            uravelled_coords_nonzero_sorted,
            obs_coords.dtype,
            [nonzero_idxs.size, self.ninput_features],
        )
        self.obs_uidxs = origidxs_nonzero[
            sortedidxs
        ]  # records the mapping from the input data to the obs_coords object
    ...
    def _process_observations(self, obs_coords, obs_values, totals=None):
        ...
        if totals is None or not np.any(totals >= 0):
            ...
            ):  # obs_value is one column with values of either 0 or 1
        ...
        if (
            ...
            obs_values.shape[1] == 2
        ):  # obs_values given as two columns: first is positive counts, second is total counts.
        ...
        poscounts, totals = self._count_observations(
            obs_coords, n_obs, poscounts, totals
        )
        ...
        if self.verbose:
            logging.debug(
                "Number of observed locations =" + str(self.obs_values.shape[0])
            )
        ...
        self.K_out = (
            {}
        )  # store the output cov matrix for each block. Reset when we have new observations.
        self.K_star_diag = {}
    ...
    def _update_jacobian(self, G_update_rate=1.0):
        ...
        if (
            G_update_rate == 1 or not len(self.G) or self.G.shape != J.shape
    ...
    def lowerbound(self, return_terms=False):
        ...
        if self.verbose:
            logging.debug(
                "DLL + logp_f: %.5f, logq_f: %.5f, logp_s-logq_s: %.5f"
                % (logp_Df, logq_f, logp_s - logq_s)
            )
    ...
    def lowerbound_gradient(self, dim):
        ...
        if (
            self.n_lengthscales == 1 or dim == -1
        ):  # create an array with values for each dimension
    ...
    def ln_modelprior(self):
        ...
        lnp_gp = (
            -gammaln(self.shape_ls)
            + self.shape_ls * np.log(self.rate_ls)
            + (self.shape_ls - 1) * np.log(self.ls)
            - self.ls * self.rate_ls
        )
    ...
    def _logpt(self):
        ...
        logrho, lognotrho, _ = self._post_sample(
            self.obs_f, np.diag(self.obs_C)[:, None], expectedlog=True
        )
    ...
    def _logpf(self):
        ...
        m_invK_m = (
            (self.obs_f - self.mu0).T.dot(self.invK * self.s).dot(self.obs_f - self.mu0)
        )
    ...
    def data_ll(self, logrho, lognotrho):
        ...
        logbc = np.log(bc)
        lpobs = np.sum(
            self.z * self.obs_total_counts * logrho
            + self.obs_total_counts * (1 - self.z) * lognotrho
        )
        lpobs += np.sum(logbc)
    ...
    def _logps(self):
        logprob_s = (
            -gammaln(self.shape_s0)
            + self.shape_s0 * np.log(self.rate_s0)
            + (self.shape_s0 - 1) * self.Elns
            - self.rate_s0 * self.s
        )
        return logprob_s
    ...
    def _logqs(self):
        lnq_s = (
            -gammaln(self.shape_s)
            + self.shape_s * np.log(self.rate_s)
            + (self.shape_s - 1) * self.Elns
            - self.rate_s * self.s
        )
    ...
    def neg_marginal_likelihood(self, hyperparams, dimension, use_MAP=False):
        ...
        logging.debug(
            "LML: %f, with Length-scales from %f to %f, after %i iterations"
    ...
    def nml_jacobian(self, hyperparams, dimension, use_MAP=False):
        ...
        gradient[
            np.isnan(gradient)
        ] = 1e-20  # remove any values that are likely to cause errors. Positive gradient to
        # encourage the lengthscales causing the problems to be increased, since they are most likely close to zero
        if use_MAP:
            logging.error(
                "MAP not implemented yet with gradient descent methods -- will ignore the model prior."
            )
    ...
    def set_training_data(
        ...
        if self.verbose:
            logging.debug(
                "GP Classifier VB: prepared for training with max length-scale %.3f and smallest %.3f"
                % (np.max(self.ls), np.min(self.ls))
            )
    ...
    def fit(
        ...
        if process_obs:
            prev_n_locs = (
                self.n_locs
            )  # how many training data points did we have before?
            self._process_observations(obs_coords, obs_values, totals)
            ...
            if use_median_ls and new_locations:
                self.ls = compute_median_lengthscales(
                    self.obs_coords, multiply_heuristic_power=0.5
                )
            ...
            mu0 is not None or K is not None
        ):  # updated mean but not updated observations
            self._init_params(
                mu0, False, K
            )  # don't reset the parameters, but make sure mu0 is updated
        ...
        if self.verbose:
            logging.debug(
                "GP Classifier VB: training with max length-scale %.3f and smallest %.3f"
                % (np.max(self.ls), np.min(self.ls))
            )
        converged_count = 0
        prev_val = -np.inf
    ...
    def _optimize(
        ...
        if process_obs:
            self._process_observations(
                obs_coords, obs_values, totals
            )  # process the data here so we don't repeat each call
        ...
        for r in range(nrestarts):
            ...
            res = minimize(
                ...
                jac=self.nml_jacobian,
                method="L-BFGS-B",
                options={
                    "maxfun": maxfun,
                    "maxiter": maxfun,
                    "ftol": 1e-3,
                    "gtol": 10 ** (-self.ninput_features - 1),
                },
            )
            ...
            if r < nrestarts - 1:
                self.ls = gamma.rvs(
                    self.shape_ls, scale=1.0 / self.rate_ls, size=len(self.ls)
                )
        ...
        logging.debug(
            "Optimal value = %.5f with hyper-parameters: %s; found using %i objective fun evals"
            % (-nlml, self.ls, nfits)
        )
    ...
    def _expec_f(self):
        ...
        for G_iter in range(self.max_iter_G):
            ...
            if np.isscalar(oldG) or self.G.shape == oldG.shape:
                ...
                diff_G = (
                    np.inf
                )  # if the shape has changed, we have changed the observations we are fitting to
            # Use a smaller update size if we get stuck oscillating about the solution
            if (
                np.abs(np.abs(diff_G) - np.abs(prev_diff_G)) < 1e-3
                and G_update_rate > 0.1
            ):
                G_update_rate *= 0.9
            if self.verbose:
                logging.debug(
                    "Iterating over G: diff was %.5f in G-iteration %i; update rate = %f"
                    % (diff_G, G_iter, G_update_rate)
                )
    ...
    def _update_f(self):
        ...
        B = solve_triangular(
            self.L, (self.z - z0), lower=True, overwrite_b=True, check_finite=False
        )
        self.A = solve_triangular(
            self.L, B, lower=True, trans=True, overwrite_b=False, check_finite=False
        )
        self.obs_f = (
            self.KsG.dot(self.A, out=self.obs_f) + self.mu0
        )  # need to add the prior mean here?
        V = solve_triangular(
            self.L, self.KsG.T, lower=True, overwrite_b=True, check_finite=False
        )
        self.obs_C = self.Ks - V.T.dot(V, out=self.obs_C)

    def _expec_s(self):
        self.old_s = self.s
        invK_expecFF = self.invK.dot(
            self.obs_C
            + self.obs_f.dot(self.obs_f.T)
            - self.mu0.dot(self.obs_f.T)
            - self.obs_f.dot(self.mu0.T)
            + self.mu0.dot(self.mu0.T)
        )
        if not self.fixed_s:
    ...
    def _check_convergence(self, prev_val):
        if self.uselowerbound and np.mod(self.vb_iter, self.conv_check_freq) == 0:
            ...
            not self.uselowerbound
            and np.mod(self.vb_iter, self.conv_check_freq) == self.conv_check_freq - 1
        ):
            diff = np.max(np.abs(self.obs_f - prev_val))
            if self.verbose:
                logging.debug(
                    "GP Classifier VB obs_f diff = %f at iteration %i"
                    % (diff, self.vb_iter)
                )
            ...
            return (
                False,
                prev_val,
            )  # not checking in this iteration, return the old value and don't converge
    ...
    def predict(
        ...
        """
        self.predict_f(
            out_feats, out_idxs, K_star, K_starstar, mu0_output, reuse_output_kernel
        )
    ...
    def predict_grid(self, nx, ny, variance_method="rough", mu0_output=None):
        nout = nx * ny
        outputx = np.tile(
            np.arange(nx, dtype=np.float).reshape(nx, 1), (1, ny)
        ).reshape(nout)
        outputy = np.tile(
            np.arange(ny, dtype=np.float).reshape(1, ny), (nx, 1)
        ).reshape(nout)
        return self.predict(
            out_feats=[outputx, outputy],
            variance_method=variance_method,
            mu0_output=mu0_output,
        )

    def _get_training_cov(
        self,
    ):
    ...
    def predict_f(
        ...
        if K_star is not None and K_starstar is not None:
            ...
            if (
                not reuse_output_kernel
                or self.out_feats is None
                or np.any(out_feats != self.out_feats)
                or self.K_star is None
            ):
                if (
                    out_feats.shape[0] == self.ninput_features
                    and out_feats.shape[1] != self.ninput_features
                ):
                ...
                self.K_star = self.kernel_func(
                    out_feats_arr,
                    self.ls,
                    self._get_training_feats(),
                    operator=self.kernel_combination,
                )
                if full_cov:
                    self.K_starstar = self.kernel_func(
                        out_feats_arr,
                        self.ls,
                        out_feats_arr,
                        operator=self.kernel_combination,
        ...
        self.f, self.v = self._expec_f_output(
            Ks_starstar, Ks_star, self.mu0_output, full_cov, reuse_output_kernel
        )

        if full_cov:
            v = np.diag(
                self.v
            )  # self.v is full covariance. Get the diags to check it is okay.
        else:
            v = self.v
        if np.any(v < 0):
            logging.error(
                "Negative variance in GPClassifierVB._predict_f(), %f" % np.min(v)
            )

        return self.f, self.v

    def _expec_f_output(
        self, Ks_starstar, Ks_star, mu0, full_cov=False, reuse_output_kernel=False
    ):
        """
