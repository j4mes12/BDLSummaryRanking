...
class LoadEmbeddings:
    def __init__(
        self, filepath, data_path, vocab_size, embedding_size=300, binary_val=True
    ):
        self.vocab_dict = {}
    ...
    def loadEmbeddings(self, filepath, data_path, vocab_size, binary_val):
        ...
        self.W = np.memmap(
            os.path.normpath("%s/embed.dat" % data_path),
            dtype=np.double,
            mode="r",
            shape=(vocab_size, self.embedding_size),
        )
        with codecs.open(
            os.path.normpath("%s/embed.vocab" % data_path), "r", "utf-8"
        ) as f:
...
if __name__ == "__main__":
    ...
    if language == "english":
        embeddPath = os.path.normpath(
            "%s/embeddings/english/GoogleNews-vectors-negative300.bin.gz" % (data_path)
        )
    ...
    if language == "german":
        embeddPath = os.path.normpath(
            "%s/embeddings/german/2014_tudarmstadt_german_50mincount.vec" % (data_path)
        )
