        ...
        for rate in rates:
            if self.learner is None or self.tune:  # needs the vectors to init
                ...
                if self.do_dim_reduction and new_items_feat.shape[1] < 300:
                    ...
                    logging.info(
                        "Switching off dimensionality reduction as we already have fewer than 300 dimensions."
                    )
                ...
                new_items_feat = (
                    None  # only pass in the feature vectors in the first iteration
                )
...
class GPPLHRewardLearner(GPPLRewardLearner):
    def __init__(
        ...
        super(GPPLHRewardLearner, self).__init__(
...
class GPPLHsRewardLearner(GPPLHRewardLearner):
    def __init__(
        ...
        super(GPPLHsRewardLearner, self).__init__(
            steep, full_cov, heuristics, n_threads, rate=200, lspower=lspower
        )
